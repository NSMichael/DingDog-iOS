...
- (void)autoBadgeSizeWithCount:(NSInteger)badgeCount;
{
	CGSize retValue;
	CGFloat rectWidth, rectHeight;
    NSString *badgeString;
    badgeString=[NSString stringWithFormat:@"%d",badgeCount];
...
+ (CustomBadge*) customBadgeWithString:(NSString *)badgeString withStringColor:(UIColor*)stringColor withInsetColor:(UIColor*)insetColor withBadgeFrame:(BOOL)badgeFrameYesNo withBadgeFrameColor:(UIColor*)frameColor withScale:(CGFloat)scale withShining:(BOOL)shining
{
    ...
    if (IOS7_SDK_AVAILABLE) {
         return [[self alloc] initWithString:badgeString withStringColor:stringColor withInsetColor:insetColor withBadgeFrame:NO withBadgeFrameColor:frameColor withScale:scale withShining:NO];
    }
...
- (void) drawRoundedRectWithContext:(CGContextRef)context withRect:(CGRect)rect{
    ...
    //在ios7下不显示badge阴影
    if (!badgeShadowHide && !IOS7_SDK_AVAILABLE) {
...
+ (CustomBadge*)customBadgeWithWhiteFrame:(NSString *)badgeString{
...
/**找布专用样式*/
+ (CustomBadge*)customBadgeWithString_zhaobu:(NSString *)badgeString{
