...
@interface EXTagSelectedViewController () <UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, TTGTextTagCollectionViewDelegate>
...
@implementation EXTagSelectedViewController
...
- (void)onRightBarButtonClicked:(id)sender {
    ...
    [self dismissViewControllerAnimated:YES completion:^{
        if (weakSelf.exTagSelectedBlock) {
            weakSelf.exTagSelectedBlock(weakSelf.selectedArray);
        }
    }];
}
...
- (UIView *)configHeaderView {
    ...
    if (!_headerView) {
        ...
        _lblExTags.frame = CGRectMake(80, 8, kScreen_Width-80-15, 40);
        _lblExTags.textAlignment = NSTextAlignmentLeft;
        _lblExTags.font = kFont14;
        _lblExTags.numberOfLines = 2;
        _lblExTags.textColor = [UIColor colorWithRGB:0xDD002F];
        [_headerView addSubview:_lblExTags];
    ...
    _lblExTags.text = [self getSelectedTagTextWithArray:_selectedArray];
...
- (void)textTagCollectionView:(TTGTextTagCollectionView *)textTagCollectionView didTapTag:(NSString *)tagText atIndex:(NSUInteger)index selected:(BOOL)selected tagConfig:(TTGTextTagConfig *)config {
    ...
    _lblExTags.text = [self getSelectedTagTextWithArray:currentSelectedArray];
}
